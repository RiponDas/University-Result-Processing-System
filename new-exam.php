...
if(isset($_POST['submit']))
{
	...
	$year = $_POST['year'];
    $status=1;
$sql="INSERT INTO exam (Chairman, Dept, Session, Semester,  Year, status) VALUES (:chairName,:dept, :session, :semester, :year, :status)";
$query = $dbh->prepare($sql);
$query->bindParam(':chairName',$chairName,PDO::PARAM_STR);
...
if($lastInsertId)
{
$msg="Exam information added successfully";
}
else 
{
...
<label for="default" class="col-sm-2 control-label">Chairman of Examination</label>
<div class="col-sm-10">
<input type="text" name="chairName" class="form-control" id="examRoll" maxlength="20" required="required" autocomplete="off">
</div>
</div>
                                                    <div class="form-group">
...
<?php $sql = "SELECT Session from student";
$query = $dbh->prepare($sql);
$query->execute();
...
{   ?>
<option value="<?php echo htmlentities($result->s_id); ?>"><?php echo htmlentities($result->Session); ?></option>
<?php }} ?>
 </select>
                                                        </div>
                                                    </div>
...
<div class="col-sm-10">
<input type="text" name="year" class="form-control" id="examRoll" maxlength="20" required="required" autocomplete="off">
</div>
